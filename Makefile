...
# Generate dbt catalog only (creates catalog.json and manifest.json)
dbt-catalog:
...
.PHONY: postgres dbt-seed dbt-build-project dbt-run dbt-build dbt-test dbt-catalog dbt-docs superset superset-build superset-logs openmetadata-migrate openmetadata openmetadata-server openmetadata-ingestion openmetadata-logs openmetadata-ingestion-logs openmetadata-migrate-logs elasticsearch stack-up stack-down
